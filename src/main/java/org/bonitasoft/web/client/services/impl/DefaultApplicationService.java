...
public class DefaultApplicationService extends AbstractService implements ApplicationService {
	...
	public void importApplications(File applicationFile, ApplicationImportPolicy policy) {
		...
		// temporary simulate a REPLACE_DUPLICATES policy here because it is not
	...
	public Application getApplication(String applicationToken) {
		...
		return apiProvider.get(ApplicationApi.class)
				.searchApplications(new ApplicationApi.SearchApplicationsQueryParams().p(0).c(1)
		...
		try {
			...
			if (name == null || name.isEmpty()) {
				throw new IllegalArgumentException(
						format("Invalid page %s, page.properties do not contains a name attribute", pageZip.getPath()));
			}
			return getPage(name);
		}
		catch (IOException e) {
	...
	public Page getPage(String token) {
		...
		List<Page> pages = apiProvider.get(PageApi.class)
				.searchPages(new PageApi.SearchPagesQueryParams().p(0).c(1).f(singletonList("urlToken=" + token)));
		if (pages.isEmpty()) {
			// TODO: why not 404 ?
