...
public class DefaultProcessService extends AbstractService implements ProcessService {
	...
	public void importProcess(File barFile, ProcessImportPolicy policy) {
		...
		if (process.isPresent()) {
			...
			case REPLACE_DUPLICATES:
				// simulate a REPLACE_DUPLICATES policy here because it is not implemented
				// in engine side
			...
			default:
				throw new DuplicatedProcessException(barInfo.getProcessName(), barInfo.getProcessVersion(),
		...
		if (ConfigurationState.RESOLVED.equals(processDeployed.getConfigurationState())) {
			log.info("Process {}-{} is resolved. Activating process...", barInfo.getProcessName(),
					barInfo.getProcessVersion());
			String processId = processDeployed.getId();
			processApi.updateProcessById(processId,
					new ProcessUpdateRequest().activationState(ActivationState.ENABLED));
			...
			throw new ProcessActivationException(
					...
					barInfo.getProcessName(), barInfo.getProcessVersion());
		}
	}
	...
	private void deleteExistingProcess(ProcessDefinition process) {
		...
		if (ActivationState.ENABLED.equals(process.getActivationState())) {
			...
			processApi.updateProcessById(process.getId(),
					new ProcessUpdateRequest().activationState(ActivationState.DISABLED));
	...
	public Optional<ProcessDefinition> getProcess(String name, String version) {
		...
		List<ProcessDefinition> processes = apiProvider.get(ProcessApi.class).searchProcesses(
				new ProcessApi.SearchProcessesQueryParams().p(0).c(1).f(asList("name=" + name, "version=" + version)));
	...
	public List<ProcessParameter> searchProcessParameters(int page, int count, String processId) {
		log.info("Search process parameters for process {} with params: p={},c={}", processId, page, count);
		return apiProvider.get(ProcessParameterApi.class)
				.searchProcessParameters(new ProcessParameterApi.SearchProcessParametersQueryParams().p(page).c(count)
	...
	public List<ProcessResolutionProblem> getProcessProblem(int page, int count, String processId) {
		log.info("Search process problems for process {} with params: p={},c={}", processId, page, count);
		return apiProvider.get(ProcessResolutionProblemApi.class).searchProcessResolutionProblems(
				new ProcessResolutionProblemApi.SearchProcessResolutionProblemsQueryParams().p(page).c(count)
						.f(singletonList("process_id=" + processId)));
	}

	@Override
	public ProcessInstantiationResponse startProcess(String name, String version) {
		return getProcess(name, version).map(p -> startProcess(p.getId())).orElseThrow(
				() -> new NotFoundException(format("No process found for name '%s' and version '%s'", name, version)));
	}
	...
	public UserTask getUserTask(String taskId) {
		...
		return ofNullable(apiProvider.get(UserTaskApi.class).getUserTaskById(taskId)).map(userTask -> {
	...
	public List<UserTask> searchUserTask(String rootContainerId) {
		...
		// comment reported from deployer code ???: TODO human task api call instead user
		// task api
		List<UserTask> userTasks = apiProvider.get(UserTaskApi.class).searchUserTasks(
				new UserTaskApi.SearchUserTasksQueryParams().f(singletonList("rootContainerId=" + rootContainerId)));
