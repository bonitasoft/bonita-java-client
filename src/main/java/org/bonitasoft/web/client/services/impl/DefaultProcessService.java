    ...
    public void importProcess(File barFile, ProcessImportPolicy policy) {
        ...
        if (process.isPresent()) {
            ...
            switch (policy) {
                ...
                default:
                    throw new DuplicatedProcessException(bar.getProcessName(), bar.getProcessVersion(), "Policy FAIL_ON_DUPLICATES: deployment aborted.");
            }
        }
        ...
        if (ConfigurationState.RESOLVED.equals(processDeployed.getConfigurationState())) {
            ...
            throw new ProcessActivationException("Process is unresolved, maybe some process parameters have not been configured yet.", bar.getProcessName(), bar.getProcessVersion());
        }
    }
