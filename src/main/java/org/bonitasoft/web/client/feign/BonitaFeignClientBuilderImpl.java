...
public class BonitaFeignClientBuilderImpl implements BonitaFeignClientBuilder {
	...
	public BonitaClient build() {
		...
		return new BonitaFeignClient(apiClient.getBasePath(), apiProvider, loginService, applicationService, bdmService,
				userService, processService, systemService);
	}

	Feign.Builder configureFeign(Feign.Builder feignBuilder) {
		log.debug("Configuring Feign builder ...");
		return feignBuilder.client(new feign.okhttp.OkHttpClient(okHttpClient)).decode404()
	...
	ObjectMapper configureJackson(ObjectMapper objectMapper) {
		...
		return objectMapper.findAndRegisterModules().setSerializationInclusion(JsonInclude.Include.NON_NULL);
	}
	...
	OkHttpClient.Builder configureHttpClient(OkHttpClient.Builder builder) {
		...
		OkHttpClient.Builder okHttpClientBuilder = addTrustAllCertificateManagerIfNeeded(builder)
				.connectTimeout(connectTimeoutInSeconds, SECONDS).readTimeout(readTimeoutInSeconds, SECONDS)
