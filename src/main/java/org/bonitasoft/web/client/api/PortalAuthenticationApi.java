...
public interface PortalAuthenticationApi extends ApiClient.Api {
   ...
   * other <code>logout</code> method, but with the query parameters collected into a single Map
   ...
   *     <ul>
   *       <li>redirect - Setting the redirect parameter to false indicates that the service should
   *           not redirect to the login page after logging out. (optional)
   *     </ul>
   */
  @RequestLine("GET /logoutservice?redirect={redirect}")
  @Headers({
  ...
  /**
   * A convenience class for generating query parameters for the <code>logout</code> method in a
   * fluent style.
   */
