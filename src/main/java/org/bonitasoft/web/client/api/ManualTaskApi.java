...
public interface ManualTaskApi extends ApiClient.Api {
	...
	/**
	 * Create the ManualTask Create the ManualTask. Use a POST method to create a new
	 * subtask. A subtask is attached to a parent task and it needs to be immediately
	 * assigned to a user.
	...
	/**
	 * Finds ManualTasks Finds ManualTasks with pagination params and filters You can
	 * filter on: * &#x60;assigned_id&#x3D;{user_id}&#x60;: retrieve only the manual tasks
	 * assigned to the specified user. For example, retrieve the manual tasks assigned to
	 * user with id 1:
	 * &#x60;/API/bpm/manualTask?p&#x3D;0&amp;c&#x3D;10&amp;f&#x3D;assigned_id%3d1&#x60;.
	 * * &#x60;state&#x3D;skipped | ready | completed | failed&#x60; : retrieve only the
	 * manual tasks with the specified state. For example, retrieve the ready tasks:
	 * &#x60;/API/bpm/manualTask?p&#x3D;0&amp;c&#x3D;10&amp;f&#x3D;state%3dready&#x60;. *
	 * &#x60;caseId&#x3D;{case_id}&#x60;: retrieve only the manual tasks created in the
	 * specified process instances. For example, retrieve the manual tasks for the
	 * case\\_id 2:
	 * &#x60;/API/bpm/manualTask?p&#x3D;0&amp;c&#x3D;10&amp;f&#x3D;caseId%3d2&#x60;. *
	 * &#x60;parentTaskId&#x3D;{parentTask_id}&#x60;: retrieve only the manual tasks for a
	 * specific parentTask. For example, retrieve the manual tasks for the parentTask\\_id
	 * 40001:
	 * &#x60;/API/bpm/manualTask?p&#x3D;0&amp;c&#x3D;10&amp;f&#x3D;parentTaskId%3d40001&#x60;.
	 * You can search on: * name: search all manual tasks with a name that starts with the
	 * search string. For example, search for all manual tasks that have a name that
	 * starts with MySubTask:
	...
	@Headers({ "Accept: application/json", })
	List<ManualTask> searchManualTasks(@Param("p") Integer p, @Param("c") Integer c, @Param("f") List<String> f,
	 ...
	 * search string. For example, search for all manual tasks that have a name that
	 * starts with MySubTask:
	 * &#x60;/API/bpm/manualTask?p&#x3D;0&amp;c&#x3D;10&amp;s&#x3D;MySubTask&#x60;. Note,
	 * this is equivalent to the other <code>searchManualTasks</code> method, but with the
	 * query parameters collected into a single Map parameter. This is convenient for
	 * services with optional query parameters, especially when used with the
	 * {@link SearchManualTasksQueryParams} class that allows for building up this map in
	 * a fluent style.
	...
	/**
	 * Update the ManualTask by ID Update the ManualTask for the given ID. Use a PUT
	 * method to execute a subtask. Executing a subtask basically means changing its state
	 * to completed and providing an executedBy value.
	 * @param id ID of the ManualTask to return (required)
	 ...
	 */
	class SearchManualTasksQueryParams extends HashMap<String, Object> {
