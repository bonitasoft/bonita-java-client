...
public interface PageApi extends ApiClient.Api {
  ...
  /**
   * Create the Page Upload the page content using the &#x60;portal/pageUpload&#x60; . This returns
   * a temporary file name that can be used as input for this operation.
   *
   ...
   * Finds Pages Finds Pages with pagination params and filters - can search on
   * &#x60;displayName&#x60;,&#x60;description&#x60; - can filter on
   * &#x60;createdBy&#x60;,&#x60;contentType&#x60; Note, this is equivalent to the other <code>
   * searchPages</code> method, but with the query parameters collected into a single Map parameter.
   * This is convenient for services with optional query parameters, especially when used with the
   * {@link SearchPagesQueryParams} class that allows for building up this map in a fluent style.
   *
  ...
  /**
   * A convenience class for generating query parameters for the <code>searchPages</code> method in
   * a fluent style.
   */
  ...
  /**
   * Update the Page by ID Use the PUT method to update an existing custom page. To update a custom
   * page, upload the new page content using the pageUpload servlet, which returns a temporary file
   * name, and then call this API with the temporary file name.
   *
   * @param id ID of the Page to return (required)
