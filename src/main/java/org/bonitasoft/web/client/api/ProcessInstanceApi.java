...
public interface ProcessInstanceApi extends ApiClient.Api {
	 ...
	 * ProcessInstance This way of creating a process instance using this method will only
	 * work for processes in which no contract is defined. To instantiate a process with a
	 * contract, check the process instantiation resource documentation.
	 * @param body **Warning**: The attribute &#x60;variables&#x60; on the request payload
	 * is used to initialize the process variables (not BDM variables). If you want to
	 * initialize BDM variables at process instantiation, add a contract on the process
	 * and map BDM variables to the contract data. See Start a process using an
	 * instantiation contract for usage. (required)
	 ...
	 * ![edition](https://img.shields.io/badge/edition-entreprise-blue)
	 * ![edition](https://img.shields.io/badge/edition-community-brightgreen) Delete a
	 * list of ProcessInstances for the given IDs
	...
	/**
	 * Finds the Context by ProcessInstance ID Returns the Context for the given
	 * ProcessInstance ID
	 ...
	 * the {@link GetProcessInstanceByIdQueryParams} class that allows for building up
	...
	/**
	 * Finds ProcessInstances Finds ProcessInstances with pagination params and filters
	 * You can filter on: * &#x60;processDefinitionId&#x60;: The process derfinition ID *
	 * &#x60;name&#x60;: the process name * &#x60;started_by&#x60;: the ID of the user who
	 * started the process * &#x60;team_manager_id&#x60;: allow to retrieve the process
	 * instances in which all users with this manager ID ar involved) *
	 * &#x60;supervisor_id&#x60;: allow the retrived the process instances of all
	 * processes the user with this ID is supervisor of) beware you cannot use
	 ...
	 * &#x60;supervisor_id&#x60;: allow the retrived the process instances of all
	 * processes the user with this ID is supervisor of) beware you cannot use
	 * team_manager_id and supervisor_id at the same time Note, this is equivalent to the
	 * other <code>
	 * searchProcessInstances</code> method, but with the query parameters collected into
	 * a single Map parameter. This is convenient for services with optional query
	 * parameters, especially when used with the {@link SearchProcessInstancesQueryParams}
	...
	@RequestLine("GET /API/bpm/case?p={p}&c={c}&f={f}&o={o}")
	@Headers({ "Accept: application/json", })
	List<ProcessInstance> searchProcessInstances(@QueryMap(encoded = true) Map<String, Object> queryParams);

	/**
	 * A convenience class for generating query parameters for the
	 * <code>getProcessInstanceById</code> method in a fluent style.
	 */
	class GetProcessInstanceByIdQueryParams extends HashMap<String, Object> {
	 ...
	 */
	class SearchProcessInstancesQueryParams extends HashMap<String, Object> {
