...
public interface ProfessionalContactDataApi extends ApiClient.Api {
  ...
  /**
   * Finds the ProfessionalContactData by ID Returns the single ProfessionalContactData for the
   * given ID
   *
  ...
  /**
   * Finds ProfessionalContactData Finds ProfessionalContactData with pagination params and filters
   * - can order on &#x60;id&#x60; - can search on &#x60;displayName&#x60; - can filter on
   * &#x60;displayName&#x60;
   *
   ...
   * Finds ProfessionalContactData Finds ProfessionalContactData with pagination params and filters
   * - can order on &#x60;id&#x60; - can search on &#x60;displayName&#x60; - can filter on
   * &#x60;displayName&#x60; Note, this is equivalent to the other <code>
   * searchProfessionalContactDatas</code> method, but with the query parameters collected into a
   * single Map parameter. This is convenient for services with optional query parameters,
   * especially when used with the {@link SearchProfessionalContactDatasQueryParams} class that
   ...
   * @param professionalContactData Partial PersonalContactData description including the user ID
   *     (required)
   */
  ...
  void updatePersonalContactDataById(
      @Param("userId") String userId, ProfessionalContactData professionalContactData);
   ...
   * @param professionalContactData Partial ProfessionalContactData description including the user
   *     ID (required)
   */
  ...
  void updateProfessionalContactDataById(
      @Param("userId") String userId, ProfessionalContactData professionalContactData);
}
