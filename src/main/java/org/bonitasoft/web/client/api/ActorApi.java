...
public interface ActorApi extends ApiClient.Api {
  ...
  /**
   * Finds Actors Finds Actors with pagination params and filters Note, this is equivalent to the
   * other <code>searchActors</code> method, but with the query parameters collected into a single
   * Map parameter. This is convenient for services with optional query parameters, especially when
   * used with the {@link SearchActorsQueryParams} class that allows for building up this map in a
   * fluent style.
   *
  ...
  /**
   * A convenience class for generating query parameters for the <code>searchActors</code> method in
   * a fluent style.
   */
   ...
   *
   * @param id ID of the Actor to return (required)
   * @param actorUpdateRequest Fields that can be upated are &#x60;displayName&#x60; and
