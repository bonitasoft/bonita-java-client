...
public interface ProcessResolutionProblemApi extends ApiClient.Api {
  ...
  /**
   * Finds ProcessResolutionProblems Finds ProcessResolutionProblems with pagination params and
   * filters to list the problems that need to be solved before a process can be used. **Filtering
   * on the process definition ID is mandatory.**
   *
   ...
   * Finds ProcessResolutionProblems Finds ProcessResolutionProblems with pagination params and
   * filters to list the problems that need to be solved before a process can be used. **Filtering
   * on the process definition ID is mandatory.** Note, this is equivalent to the other <code>
   * searchProcessResolutionProblems</code> method, but with the query parameters collected into a
   * single Map parameter. This is convenient for services with optional query parameters,
   * especially when used with the {@link SearchProcessResolutionProblemsQueryParams} class that
