...
public interface ProcessParameterApi extends ApiClient.Api {
  ...
  /**
   * Finds the ProcessParameter by ID
   * Similar to <code>getProcessParameterById</code> but it also returns the http response headers .
   * Returns the single ProcessParameter for the given ID 
   * @param id ID of the process to get parameter from (required)
   * @param name Name of the process parameter to return (required)
  ...
  ApiResponse<ProcessParameter> getProcessParameterByIdWithHttpInfo(@Param("id") String id, @Param("name") String name);
  ...
  /**
   * Finds ProcessParameters
   * Similar to <code>searchProcessParameters</code> but it also returns the http response headers .
   * Finds ProcessParameters with pagination params and filters. 
   * @param p index of the page to display (required)
   * @param c maximum number of elements to retrieve (required)
   * @param f can filter on attributes with the format f&#x3D;{filter\\_name}&#x3D;{filter\\_value} with the name/value pair as url encoded string. (optional)
  ...
  ApiResponse<List<ProcessParameter>> searchProcessParametersWithHttpInfo(@Param("p") Integer p, @Param("c") Integer c, @Param("f") List<String> f, @Param("o") String o, @Param("s") String s);


  /**
   * Finds ProcessParameters
   * Finds ProcessParameters with pagination params and filters. 
   * Note, this is equivalent to the other <code>searchProcessParameters</code> method,
  ...
  List<ProcessParameter> searchProcessParameters(@QueryMap(encoded=true) SearchProcessParametersQueryParams queryParams);

  /**
  * Finds ProcessParameters
  * Finds ProcessParameters with pagination params and filters. 
  * Note, this is equivalent to the other <code>searchProcessParameters</code> that receives the query parameters as a map,
  * but this one also exposes the Http response headers
   ...
   ApiResponse<List<ProcessParameter>> searchProcessParametersWithHttpInfo(@QueryMap(encoded=true) SearchProcessParametersQueryParams queryParams);


   /**
   * A convenience class for generating query parameters for the
   * <code>searchProcessParameters</code> method in a fluent style.
   */
  ...
  ApiResponse<Void> updateProcessParameterByIdWithHttpInfo(@Param("id") String id, @Param("name") String name, ProcessParameterUpdateRequest processParameterUpdateRequest);
