...
class BonitaCharsetBugInterceptorTest {
	...
	@ParameterizedTest
	@ValueSource(strings = { "; charset=utf-8", ";charset=utf-8", "; charset=UTF-8", ";charset=UTF-8",
	...
	void request_should_be_cleaned() throws IOException {
		// Given
		BonitaCharsetBugInterceptor interceptor = new BonitaCharsetBugInterceptor();
		final Request mockRequest = new Request.Builder().url("http://dummy.org")
				.header(CONTENT_TYPE_HEADER, "application/json; charset=utf8").build();
	...
	void requestTemplate_should_be_cleaned() {
		...
		BonitaCharsetBugInterceptor interceptor = new BonitaCharsetBugInterceptor();
		String baseContentType = "application/json";
		RequestTemplate template = new RequestTemplate().header(CONTENT_TYPE_HEADER,
				singletonList(baseContentType + "; charset=UTF-8"));
