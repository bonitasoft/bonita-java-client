...
class BonitaLoginServiceTest {
    ...
    private Response mockResponse(int status, Map<String, Collection<String>> headers, Object body) throws JsonProcessingException {
        final Response.Builder builder = Response.builder().status(status).headers(headers).request(
                Request.create(Request.HttpMethod.POST, "http://dummy.com/login", new HashMap<>(), null, new RequestTemplate())
        );
        if (body != null) {
    ...
    void login_should_work_with_204_response() throws Exception {
        ...
        when(authenticationApi.login(anyString(), anyString(), anyString(), anyString(), anyString())).thenReturn(loginResponse);

        final Response sessionResponse = mockResponse(200, new HashMap<>(), new Session().userName(username));
