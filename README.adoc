...
* Built-in Bonita authentication support (SSO only supported through Bearer authentication with OpenID Connect SSO)
...
NOTE: When Bonita is configured for SSO with OpenID Connect, you can replace the `login` method call with a call to `setBearerToken` method and pass the access token value as parameter.
